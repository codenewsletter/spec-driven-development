...
This module explains what spec-driven development is, why it is crucial, and how to tell when it will pay off.
...
Thoughtworks Distinguished Engineer Birgitta Böckeler [defines](https://martinfowler.com/articles/exploring-gen-ai/sdd-3-tools.html) spec-driven development (SDD) as writing a spec before you write code with AI, with the spec then the source of truth for both.
...
Her test of GitHub's [Spec Kit](https://github.com/github/spec-kit) shows the gap. GitHub describes specs as living artifacts, but Spec Kit creates a branch for every spec. To Böckeler that makes it spec-first, because the spec lives for one change request, not for the life of a feature.
...
GitHub Principal Product Manager Den Delimarsky [argues](https://github.blog/ai-and-ml/generative-ai/spec-driven-development-with-ai-get-started-with-a-new-open-source-toolkit/) that the problem is not the agent's coding ability. It is our approach. We treat coding agents like search engines when we should treat them like literal-minded pair programmers.

Agents excel at pattern recognition, but they still need unambiguous instructions.
...
When requirements, constraints, and edge cases live only in prompts, teams get fast output without a durable source of truth. Microsoft lists architectural drift, code drift, inconsistent implementations, harder reviews, and rework as the results.

### Faster Developers Did Not Make Faster Teams

Microsoft Digital, the company's IT organization, [found after a year of ad-hoc AI adoption](https://www.microsoft.com/insidetrack/blog/engineering-the-frontier-firm-sharing-our-ai-native-approach-to-software-development/) that individual developers got faster and the gains stopped there.
...
Microsoft [defines what goes in it](https://www.microsoft.com/insidetrack/blog/engineering-the-frontier-firm-sharing-our-ai-native-approach-to-software-development/) as a shared set of architectural principles, governance requirements, security standards, and development limitations that everyone adheres to.
...
Thoughtworks [sets a clear bar](https://www.thoughtworks.com/insights/blog/agile-engineering-practices/spec-driven-development-unpacking-2025-new-engineering-practices). A spec explicitly defines the external behavior of the target software. That means input and output mappings, preconditions and postconditions, invariants, constraints, interface types, integration contracts, and sequential logic or state machines.

Thoughtworks puts technical requirements in a third place. Architectural style and constraints go in rules files such as AGENTS.md. The agent then generates code from the finalized spec against those rules.
...
Engineering Leadership newsletter author Gregor Ojstersek [says](https://newsletter.eng-leadership.com/p/how-to-do-spec-driven-development) SDD is overhead for a small change, like a refactor of one file or a small adjustment to a feature. Prompt the agent and review the code instead.
...
Matt Pocock, who runs AI Hero, [says the whole trigger](https://www.aihero.dev/skills-to-spec) for a spec is a build too big for one agent session, one that has to survive a split across several.
...
Böckeler asked a spec tool to fix a small bug, and [the workflow turned it into 4 user stories with 16 acceptance criteria](https://martinfowler.com/articles/exploring-gen-ai/sdd-3-tools.html). Her verdict was a sledgehammer to crack a nut.
